...
| ---------------- | --------------------------------------------------- | ---------- | --------------- | ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 6/4/24 11:19 AM  | [train_1.1](Defect-Detection-exp/train_1.1)         | 0.767      | 0.705           | 3012018    | 采用原始`yolov8n.pt`及原始超参数。                           | -                                                            |
...
| 6/4/24 09:38 PM  | [train_2.1](Defect-Detection-exp/train_2.1)         | 0.776      | 0.683           | 3012018    | 原始`yolov8n.pt`的基础上采用 [tune1](Defect-Detection-exp/tune1) 的超参数调优结果。 | mAP50获得了一定的提升，较[train_1.1](Defect-Detection-exp/train_1.1)增长了0.009，但存在轻微的过拟合问题。 |
| 6/6/24 07:22 AM  | [train_2.2](Defect-Detection-exp/train_2.2)         | 0.769      | 0.674           | 3012018    | [tune2](Defect-Detection-exp/tune2)调优过程得到的[最佳权重](Defect-Detection-exp/tune2/tune/weights/best.pt) 和[最佳超参数组合](Defect-Detection-exp/tune2/tune/best_hyperparameters.yaml) 上继续训练。 | 在第114个Epoch时触发了early stopping，mAP50未提高，loss的收敛情况变差，存在严重的过拟合问题。 |
| 6/6/24 08:30 AM  | [train_2.3](Defect-Detection-exp/train_2.3)         | 0.774      | 0.671           | 3012018    | [tune2](Defect-Detection-exp/tune2)调优过程得到的[最佳超参数组合](Defect-Detection-exp/tune2/tune/best_hyperparameters.yaml) 和[train_2.1](Defect-Detection-exp/train_2.1)得到的[最佳权重](Defect-Detection-exp/train_2.1/weights/best.pt) 上继续训练。 | 相当于运用“早停法”，仍然存在过拟合问题。                     |
| 6/6/24 09:09 AM  | [train_2.1.1](Defect-Detection-exp/train_2.1.1)     | 0.782      | 0.689           | 3012018    | [train_2.1](Defect-Detection-exp/train_2.1)的基础上epoch减小至100。 | 过拟合问题得到缓解，训练时长缩短，模型也回到更加轻量的水平。 |
| 6/6/24 09:39 AM  | [train_2.1.1.1](Defect-Detection-exp/train_2.1.1.1) | 0.785      | 0.698           | 3012018    | [train_2.1.1](Defect-Detection-exp/train_2.1.1) 的基础上batch增大至24。 | 出现略微过拟合。                                             |
| 6/6/24 01:08 PM  | [train_3.1](Defect-Detection-exp/train_3.1)         | 0.785      | 0.7             | 3012018    | 采用[tune3](Defect-Detection-exp/tune3)得到的[最佳超参数组合](Defect-Detection-exp/tune3/tune/best_hyperparameters.yaml) 对原始`yolov8n.pt`训练100个epoch。 | 略微过拟合，波动幅度仍然较大。                               |
| 6/12/24 03:30 PM | [train_3.2](Defect-Detection-exp/train_3.2)         | 0.777      | 0.685           | 3012018    | 采用[tune3](Defect-Detection-exp/tune3)得到的[最佳超参数组合](Defect-Detection-exp/tune3/tune/best_hyperparameters.yaml) 的基础上微调过的[hyp.yaml](hyp_modified.yaml)（增加`flipud=0.5`）进行训练，调整图像尺寸为`320`。 | 过拟合问题得到缓解，训练速度变快，但是训练过程loss曲线波动增大，需要对新增加数据增强参数进行调优。💡同时可以发现`rolled-in_scale`类别的检测效果有所改善。 |
| 6/12/24 04:50 PM | [train_3.3](Defect-Detection-exp/train_3.3)         | 0.708      | 0.661           | 3012018    | 在[train_3.2](Defect-Detection-exp/train_3.2)的基础上增加更多数据增强选项，如`degrees`、`mixup`… | 精度波动过大，mAP50下降。                                    |
| 6/12/24 05:12 PM | [train_3.4](Defect-Detection-exp/train_3.4)         | 0.735      | 0.666           | 3012018    | 在[train_3.3](Defect-Detection-exp/train_3.3)的基础上关闭`degrees`增强选项，图像尺寸减小为`256`（排除图像噪声干扰）。 | 精度波动仍然较大。但是考虑到`mixup`选项与test数据集的特性一致，在本次调优中予以保留。 |
| 6/14/24 12:29 PM | [train_3.2.1](Defect-Detection-exp/train_3.2.1)     | 0.762      | 0.681           | 3012018    | 采用[tune6](Defect-Detection-exp/tune6)的调优结果训练100个epoch。 |                                                              |
| 6/14/24 01:13 PM | [train_3.2.2](Defect-Detection-exp/train_3.2.2)     | 0.776      | 0.666           | 3012018    | 采用[tune6](Defect-Detection-exp/tune6)的调优结果训练200个epoch。 | 稍微有些过拟合。                                             |
| 6/14/24 01:35 PM | [train_3.2.3](Defect-Detection-exp/train_3.2.3)     | 0.767      | 0.692           | 3012018    | 采用[tune6](Defect-Detection-exp/tune6)的调优结果训练150个epoch。 |                                                              |

...
def unzip_file(file, path=None, exclude=(".DS_Store", "__MACOSX"), exist_ok=False, progress=True):
    ...
    with ZipFile(file) as zipObj:
        ...
        if len(top_level_dirs) > 1 or (len(files) > 1 and not files[0].endswith("/")):
            ...
            path = extract_path = Path(path) / Path(file).stem  # i.e. ../NEU_DET_data/coco8
        else:
            # Zip has 1 top-level directory
            extract_path = path  # i.e. ../NEU_DET_data
            path = Path(path) / list(top_level_dirs)[0]  # i.e. ../NEU_DET_data/coco8
